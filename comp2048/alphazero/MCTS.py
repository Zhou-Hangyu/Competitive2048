# Incorporated from https://github.com/suragnair/alpha-zero-general/blob/master/MCTS.py
...
class MCTS:
    ...
    def search(self, canonicalBoard, score):
        """
        ...
        if s not in self.Es:
            self.Es[s] = self.game.getGameEnded(canonicalBoard, score, 1)
        if self.Es[s] != 0:
            # terminal node
            return -self.Es[s]
        ...
        if s not in self.Ps:
            ...
            valids = self.game.getValidMoves(canonicalBoard, score, 1).cpu()
