...
from comp2048.models.Comp2048Players import RandomPlayer, GreedyComp2048Player
...
class Coach:
    ...
    def __init__(self, game, nnet, args, seed=None, device="cpu"):
        ...
        self.device = device
        self.mcts = MCTS(
            self.game, self.nnet, self.args, seed=self.seed, device=self.device
        )
    ...
    def executeEpisode(self):
        ...
        while True:
            ...
            board, score, self.curPlayer = self.game.getNextState(
                board, score, self.curPlayer, action
            )
    ...
    def learn(self):
        ...
        record_train[
            "win_rate_mcts"
        ] = []  # win rate against original mcts baseline TODO
        ...
        for i in range(1, self.args.numIters + 1):
            ...
            pmcts = MCTS(
                self.game, self.pnet, self.args, seed=self.seed, device=self.device
            )
            ...
            if self.args.baseline:
                ...
                arena = Arena(
                    lambda x, score: torch.argmax(
                        nmcts.getActionProb(x, score, temp=0)
                    ).item(),
                    rp,
                    self.game,
                    display=lambda board: self.game.display(board),
                )
                nwins, rpwins, draws = arena.playGames(
                    self.args.arenaCompare, verbose=self.args.verbose
                )

                log.info(
                    "NEW/RANDOM WINS : %d / %d ; DRAWS : %d" % (nwins, rpwins, draws)
                )
                ...
                nwins, gpwins, draws = arena.playGames(
                    self.args.arenaCompare, verbose=self.args.verbose
                )

                log.info(
                    "NEW/GREEDY WINS : %d / %d ; DRAWS : %d" % (nwins, gpwins, draws)
                )
            ...
            arena = Arena(
                lambda x, score: torch.argmax(
                    pmcts.getActionProb(x, score, temp=0)
                ).item(),
                lambda x, score: torch.argmax(
                    nmcts.getActionProb(x, score, temp=0)
                ).item(),
